...
// QMI8658 @ 0x6B on the shared I2C bus. Same chip and accel->quadrant logic as
// the S3 AMOLED-2.16 sibling. The rotation strip lives in internal SRAM here
// (no PSRAM) — see display.cpp. If a quadrant rotates the wrong way on this
// carrier, adjust the axis signs/swap in accel_to_rotation (IMU mounting vs
// the C6 panel's MADCTL-0x00 default may differ from the S3 board).
...
void imu_hal_tick(void)
{
    if (!imu_ok)
        return;
    uint32_t now = millis();
    if (now - last_poll_ms < IMU_POLL_MS)
        return;
    last_poll_ms = now;
