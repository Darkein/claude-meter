...
#
# Records "working", EXCEPT for AskUserQuestion: that tool's own PreToolUse hook
# sets "asking" (Claude is blocked on a user choice), and both hooks fire for an
# AskUserQuestion event — so we must NOT overwrite "asking" with "working" here.
#
...
[ "$TOOL" = "AskUserQuestion" ] && exit 0   # let the asking hook own this event
